    ...
    public function getRemainData(Request $req)
    {
        ...
        $depart = $req->input('depart');
            ...
            $leaves = \DB::table('leaves')
                        ->select(
                            'leave_person',
                            \DB::raw("count(case when (leaves.leave_type='1') then leaves.id end) as ill_times"),
                            \DB::raw("sum(case when (leaves.leave_type='1') then histories.ill_days end) as ill_days"),
                            \DB::raw("count(case when (leaves.leave_type='2') then leaves.id end) as per_times"),
                            \DB::raw("sum(case when (leaves.leave_type='2') then histories.per_days end) as per_days"),
                            \DB::raw("count(case when (leaves.leave_type='3') then leaves.id end) as vac_times"),
                            \DB::raw("sum(case when (leaves.leave_type='3') then histories.vac_days end) as vac_days"),
                            \DB::raw("count(case when (leaves.leave_type='4') then leaves.id end) as lab_times"),
                            \DB::raw("sum(case when (leaves.leave_type='4') then histories.lab_days end) as lab_days"),
                            \DB::raw("count(case when (leaves.leave_type='5') then leaves.id end) as hel_times"),
                            \DB::raw("sum(case when (leaves.leave_type='5') then histories.hel_days end) as hel_days"),
                            \DB::raw("count(case when (leaves.leave_type='6') then leaves.id end) as ord_times"),
                            \DB::raw("sum(case when (leaves.leave_type='6') then histories.ord_days end) as ord_days")
                        )
                        ->leftJoin('histories', function($join) use ($year) {
                            $join->on('histories.person_id', '=', 'leaves.leave_person');
                            $join->where('histories.year', '=', $year);
